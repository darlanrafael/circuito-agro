...
export function Dashboard({ events }: Props) {
  ...
  useEffect(() => {
    ...
    if (ids.length === 0) { setCostsTotal(0); return; }
    Promise.all(ids.map((id) => fetch(`/api/events/costs?event_id=${id}`).then((r) => r.json())))
      .then((lists) => {
        const all = lists.flat().filter(Boolean) as { valor: number }[];
        setCostsTotal(all.reduce((s, c) => s + (c.valor || 0), 0));
      })
    ...
    <div style={{ minHeight: "100vh", background: "#0d0d0d" }}>
      ...
      <div className="mx-auto max-w-7xl px-4 py-3 sm:px-5 sm:py-4 lg:px-8 lg:py-6">
        ...
        <section className="mb-6 sm:mb-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-1.5 sm:gap-3 lg:gap-4">
            ...
            <IndicatorCard title="ROI Real" value={roiReal > 0 ? `${roiReal.toFixed(1)}x` : "—"} trend={roiReal >= 1 ? "up" : roiReal > 0 ? "down" : "neutral"}
              icon={<svg className="h-4 w-4 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>} />
